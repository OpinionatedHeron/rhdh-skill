...
Show the full command inside `MutationPlan/v1`, including parameters, target job,
risks, rollback/abort behavior, and verification. Ask for explicit approval before
omitting `--dry-run`:
...
- Missing or expired authentication returns `SetupRequired/v1` with `nextCommand: "/setup-rhdh-skills openshift-ci"`; the script never performs login
