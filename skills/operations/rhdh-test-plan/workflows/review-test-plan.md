...
| Requirement | Check |
|-------------|-------|
| **Jira** | Invoke `rhdh-jira` and require the needed capability in `JiraCapabilities/v1` |
| **Google Sheets** | `python scripts/check_gsheets.py --json` → `"capability_ready": true` |

If Jira capability is missing, surface `SetupRequired/v1` from `rhdh-jira` and
...
Invoke the named skill `rhdh-jira` for `TICKET-ID` and consume an enriched
...
Invoke the named skill `rhdh-release` and prefer `ReleaseSchedule/v1`. The local
...
Invoke the named skill `rhdh-platform-support` for each required product and
...
Invoke `rhdh-jira` with a `MutationPlan/v1` that includes the target ticket, the
...
Invoke `rhdh-jira` with a `MutationPlan/v1` containing the exact comment. The user
...
Collect all accepted child tasks into one `rhdh-jira` `MutationPlan/v1`, with each
...
- **Credential safety**: Jira access must use `rhdh-jira` and its authenticated adapter. Never
