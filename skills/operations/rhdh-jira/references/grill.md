...
Before any create/grill path (`to-feature`, `to-epic`, `to-issue`), invoke the
named skill `grilling`. If the host reports that it is unavailable, hard-stop and
return:
...
The user-invoked setup router owns detection and installation. Do not probe host
skill directories or run an installer from this skill. Resume the create branch
only after the human completes setup and `grilling` can be invoked by name.
