...
Then set priority, component, and story points through the authenticated host adapter. Include this
redacted payload in the approved mutation plan:
...
If a parent Epic exists, link through the same adapter. Same-project RHIDP→RHIDP uses the native
`parent` field:
