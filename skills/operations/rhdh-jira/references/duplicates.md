...
5. Normalize results into `JiraQueryResult/v1`.

Use the authenticated host GraphQL adapter only when relationship-heavy enrichment is required and
the capability is ready. Never construct credentials or raw HTTP calls.
...
with an explicitly distinct scope. During an audit, report findings only; do not link or close issues
without a separate approved `MutationPlan/v1`.
