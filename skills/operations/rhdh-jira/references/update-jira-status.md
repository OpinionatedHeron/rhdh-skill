...
Use `acli` through its native credential store. Use the authenticated host adapter only when the
required relationship data is not available from `acli`.
...
Fetch richer state when needed:
