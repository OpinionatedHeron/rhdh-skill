...
```bash
uv run scripts/release.py --json team-breakdown {{RELEASE_VERSION}}
```
...
Invoke `rhdh-jira` with the `open_issues_by_team` JQL from
`references/jql-release.md`, substituting `{{RELEASE_VERSION}}` and the team's
`{{CLOUD_ID}}`. Consume `JiraQueryResult/v1` for both the count and, when full
issue details are requested, the issue fields.
...
- `rhdh-jira` owns paging for bulk searches; check `truncated` in `JiraQueryResult/v1` before reporting a total.
