...
Use `RhdhContext/v1` established by `SKILL.md`. Your `@backstage/*`
dependency versions must match the target RHDH version. Mismatches cause
runtime errors — most commonly "Cannot read properties of undefined."
...
plugins not yet migrated. For migration, follow `workflows/migrate-nfs.md`.
...
## Before Producing ChangeHandoff/v1
...
public exports. Include changed reports in `ChangeHandoff/v1.data.files`;
CI checks them. All public
exports need `/** @public */` JSDoc with a description (not just the bare tag).

Changesets, staging, and signed-off commits belong to `/rhdh-pull-request`.
Provide a concise `release_note` and verification evidence in the artifact; do
not create publication state here.
