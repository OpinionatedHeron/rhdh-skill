...
plain bug report. It always verifies the fix before producing a
`ChangeHandoff/v1`. The legacy `--no-verify` flag means "skip the human review
pause"; it never skips technical verification.
...
For Jira, prefer invoking `/rhdh-jira` with the key and request an
`IssueContext/v1`. If unavailable, use authenticated `acli` without reading token
files into context. For GitHub, follow `references/github-input.md`. Record:
source, key or number, URL, title, description, labels/component, reproduction
steps, expected result, and attachments.
...
its resulting constraints before implementation. If unavailable, return
`SetupRequired/v1 {missing: [grilling], nextCommand: /setup-rhdh-skills}`.
...
## 7. Clean diagnostic files and build the artifact
...
createdAt: ISO-8601
data:
  summary: concise user-visible fix summary
  files: []
  verification:
    contract: VerificationEvidence/v1
    data: {subject: bug-fix-id, checks: [], result: pass | fail | partial}
  repository: owner/repo
  workspace: path
  changeKind: bug-fix
  issue: {source: jira | github | none, keyOrNumber: null, url: null, title: null}
  rootCause: concise evidence-backed explanation
  recordings: {before: null, after: null}
  testPlan: []
```

Derive `data.testPlan` from positive reproduction steps, expected behavior,
and one regression check. Verify `data.files` against `git status`; do not stage.
...
After a successful `PullRequestReceipt/v1`, remove the exact
`e2e-tests/_repro-artifacts/` directory and report cleanup. Never remove it
before recording upload has succeeded or been handed back for manual action.
...
| `references/github-input.md` | Fetching GitHub issue context |
