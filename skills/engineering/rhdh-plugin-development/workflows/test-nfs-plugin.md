...
2. Export for dynamic loading with this skill's deterministic script:
...
1. Build `ChangeHandoff/v1` with the exact exported package, plugin
   configuration, required variable names, and checks in `data`.
2. Invoke `/rhdh-local` with it and consume `VerificationEvidence/v1`.
...
2. Add these variable names and values to `ChangeHandoff/v1.data`, invoke
   `/rhdh-local`, and consume `VerificationEvidence/v1`.
