...
target, the established `RhdhContext/v1` is authoritative; do not upgrade past
the Backstage version shipped by that RHDH release.
...
- If the user chose **"latest for my RHDH version"**: use the established
  `RhdhContext/v1`. If unresolved, invoke `/rhdh-context` for that artifact or
  ask the user for both target versions.
...
If NFS migration directed you here, also apply `references/api-changes.md`.
