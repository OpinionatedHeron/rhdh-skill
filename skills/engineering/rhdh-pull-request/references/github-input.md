...
Reference for resolving GitHub issue input and fetching publication context.
...
2. If the input matches `#\d+`, resolve the repo from `git remote -v` using `references/repo-profiles.md`.
...
The commands below are payload patterns, not authorization. Before any comment
or label edit, include the exact command, repository, issue, and body/label in
`MutationPlan/v1`, obtain approval of its material hash, and return
`MutationReceipt/v1` after execution.
