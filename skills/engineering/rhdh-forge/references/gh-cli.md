...
Read patterns for pull requests, checks, workflow runs, and repository content,
plus the behaviours that catch a caller out. Everything here is read-only; see
the mutation boundary in `SKILL.md` before any payload is executed.
...
For a full review context artifact rather than an ad-hoc read, invoke
`/rhdh-pr-review`, which owns the deterministic `ReviewContext/v1` fetch.
