...
`scripts/helper.py` in `SKILL.md`. For another skill, invoke its stable name and
consume a declared artifact; never reference its path.

## Routing Failures

### Ambiguous route

**Symptom:** A selected domain skill guesses a branch even though the request
does not provide enough information.

**Fix:** If the request already selects a branch, proceed. Otherwise ask one
focused routing question and then load the matching same-skill reference:
...
Do not turn a model-invoked skill into a catalog of other promoted skills.
