...
## Cohesive domain router

### When to use

Use a routing table when one promoted skill has multiple branches that share a
domain model, prerequisites, and completion contract. The table is a
progressive-disclosure device inside that skill, not a second discovery system.

Do not create a broad model-invoked catalog, orientation menu, or router whose
only behavior is opening other skills. Give independently triggered outcomes
their own promoted skill and compose them by stable name and artifact. Reserve
human wayfinding and setup for the repository's approved human entry points.

### Evolving from reference files to a router

Skills often start with standalone reference files loaded on demand. Add a
route table when:

- You have multiple distinct operations and expect to add more
- Operations share setup gates, auth, or domain context
- The selected skill needs deterministic branch selection after activation

Migration path:

1. Keep existing reference files as-is — they become command references
2. Add the router table and routing rules to SKILL.md
3. Create `scripts/command-metadata.json` as the source of truth when scripts
   or generated interfaces also consume command metadata
4. Add a "Common Workflows" section that maps user intents to commands

Do not add a router merely because the skill has several reference files. The
deletion test is whether removing the route would spread meaningful branch
selection across the skill, not whether a line-count threshold was crossed.

### Structure

The `SKILL.md` contains the smallest useful equivalents of these sections:

1. principles that apply to every branch;
2. intake only when the request does not already identify a branch;
3. a routing table mapping intent to same-skill references or workflows; and
4. conditional pointers stating exactly when to load each reference.

Use ordinary Markdown unless a target harness has a demonstrated need for an
additional structure. Never make XML tags part of a tested public interface.

Put branch-only detail in one-level-deep references or workflows. Keep entry
gates, shared invariants, artifact interfaces, and completion criteria in
`SKILL.md`.
...
Wait only when the request did not already choose a branch.
...
| Request does not match | Ask one focused routing question |
```
...
return `SetupRequired/v1` with the exact `/setup-rhdh-skills <route>` command,
then resume the original task after the human completes setup.
```
...
Present the complete `MutationPlan/v1`, including ordered adapter operations and the canonical
material hash: "Review this exact plan and hash before creating. [approve / edit / cancel]"

Approval applies only to that material hash. After adapter execution, emit `MutationReceipt/v1`
with the same plan ID and hash plus exactly one ordered, identity-matched `completed`, `failed`, or
`skipped` outcome for every planned operation. A preview without the plan, hash, and receipt is not
the mutation gate.
