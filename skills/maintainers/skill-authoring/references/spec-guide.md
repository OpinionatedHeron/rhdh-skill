...
Use concise Markdown headings to separate principles, intake, routing, and
conditional references when those sections are needed:
...
The section names are not an interface and should not be prose-tested.
...
- When OAuth or installation requires human interaction, return `SetupRequired/v1`; the explicit
  `/setup-rhdh-skills` entry may guide browser consent.
