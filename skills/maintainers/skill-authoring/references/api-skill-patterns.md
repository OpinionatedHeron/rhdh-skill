...
3. If the required adapter is unavailable, emit `SetupRequired/v1` with
   `nextCommand: "/setup-rhdh-skills <route>"`.
...
3. If neither capability is ready, emit `SetupRequired/v1`; do not construct a parallel raw HTTP
   authentication path.
```

Subcommands name the required capability and defer authentication to that adapter. They do not
repeat setup or transport details.
